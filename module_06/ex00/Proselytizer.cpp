...
/*   Updated: 2022/08/27 17:35:29 by ajazbuti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Proselytizer.hpp"

Proselytizer::Proselytizer(void):
...
Proselytizer::Proselytizer(char *str):
   	_arg(str), _isNan(false), _char(0), _int(0), _float(0), _double(0) 	{
//	std::cout << "Constructor with an argument" << std::endl;
	if (_arg.empty())
		throw (Proselytizer::NaughtyInputException());
}

Proselytizer::Proselytizer(Proselytizer const & src):
	_arg(src._arg), _char(src._char), _int(src._int),
	_float(src._float), _double(src._double)	{
	std::cout << "Proselytizer Clone(copy) contructor" << std::endl;
}

Proselytizer	&Proselytizer::operator=(Proselytizer const & rhs)	{
	if (this == &rhs)
		return (*this);
	this->_arg = rhs._arg;
...
std::string const & Proselytizer::getArg(void) const	{
	return (_arg);
}

char const *Proselytizer::NaughtyInputException::what() const throw()	{
...
void	Proselytizer::fromChar(void)	{
...
void	Proselytizer::fromInt(void)	{
	try	{
		_int = std::stoi(_arg);
...
void	Proselytizer::fromFloat(void)	{
...
void	Proselytizer::fromDouble(void)	{
...
void	Proselytizer::proselytize(void)	{
	int i = analyze();
	switch (i)	{
		case (CHAR):
			fromChar();
